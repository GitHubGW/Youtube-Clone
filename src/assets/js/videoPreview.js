...
const handleThumbnailVideo = () => {
  if(thumbnailVideo.length>0){
    for(let i=0; i<thumbnailVideo.length; i++){
      thumbnailVideo[i].addEventListener('mouseover', function() { thumbnailVideo[i].play() });
      thumbnailVideo[i].addEventListener('mouseout', function() { thumbnailVideo[i].pause() });
    }
  }
}
...
const handleSearchVideo = () => {
  if(searchVideo.length>0){
    for(let i=0; i<searchVideo.length; i++){
      searchVideo[i].addEventListener('mouseover', function() { searchVideo[i].play() });
      searchVideo[i].addEventListener('mouseout', function() { searchVideo[i].pause() });
    }
  }
}
...
const handleVideoBlockThumbnail = () => {
  if(videoBlockThumbnail.length>0){
    for(let i=0; i<videoBlockThumbnail.length; i++){
      videoBlockThumbnail[i].addEventListener('mouseover', function() { videoBlockThumbnail[i].play() });
      videoBlockThumbnail[i].addEventListener('mouseout', function() { videoBlockThumbnail[i].pause() });
    }
  }
}
